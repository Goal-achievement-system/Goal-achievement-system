...
### 일반 목표등록 기능
1. 회원은 목표를 등록할 때 "목표 이름, 목표 카테고리, 상세 내용, 기간 제한, 보증금, 달성(실패)시 추가금 지급방식" 을 입력합니다.
...
### 지속 목표 등록기능 ==> 
1. 지속목표란, 일정 기간동안 매일 지속적으로 인증해야 하는 목표를 의미합니다. 
2. 지속목표를 등록할 때 "목표이름, 상세 내용, 기간, 보증금"을 입력합니다.
    - 기간은 최소 1주일부터 1주일 단위로 선택할 수 있습니다.
3. 보증금은 최소 만원 부터 최대 100만원까지 만원단위로 설정 가능합니다. 단, 보증금은 계정에 충전되어있는 만큼만 설정할 수 있습니다.

4. 목표등록과 동시에 보증금이 차감됩니다.

4. 지속목표의 보상기준은 다음과 같습니다.
    1. 달성률 75% 미만 : 없음
    2. 달성률 75% 이상 : 보증금 환급
    3. 달성률 100% : 보증금 + 보증금의 10% (단, 1원 미만은 모두 버립니다)
5. 보상은 최종검증이 된 이후 회원이 결과를 인정하면 지급됩니다.
